...
#### Region
Sum over all cities in each province.
```{r}
school_count <- dt_region[, .(school_count =sum(elementary_school_count)), by = province]

nursing_count <- dt_region[, .(nursing_count =sum(nursing_home_count)), by = province]
```
Observation: province Korea. Is it the whole counrty?


```{r}
ggplot(data = school_count, aes(x=province, y=school_count)) + geom_point()
ggplot(data = nursing_count, aes(x=province, y=nursing_count)) + geom_point()
```
